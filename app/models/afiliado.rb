class Afiliado < ActiveRecord::Base
   attr_accessible :apellido_nombre, :documento_nro, :documento_tipo, :beneficio,
                   ...
                   :codigo_provincia, :codigo_localidad, :codigo_departamento, :activo
  ...
  def self.import_bajas(file, estado=true)
      ...
      (2..spreadsheet.last_row).each do |i|
            ...
            afiliado_row[:tipo_documento]    = row[8]
            afiliado_row[:numero_documento]  = row[9].to_s.split(".")[0]

            afiliado_row[:fecha_nacimiento]  = row[10]
            afiliado_row[:fecha_alta]        = row[11]
            afiliado_row[:fecha_baja]        = row[12]
            afiliado_row[:motivo]        = row[13]
            afiliado_row[:codigo_provincia]       = row[14]
            afiliado_row[:codigo_departamento]    = row[15]
            afiliado_row[:codigo_localidad]       = row[26]
